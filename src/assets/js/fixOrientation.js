 ...
 */
export const fixOrientation = function (win) {
  var meta = {}
  var timer
  ...
  var ffstr = hstyle['font-family']
  meta.font = ffstr

  // automatically load css script
  function loadStyleString (css) {
    var _style = document.createElement('style')
    var _head = document.head ? document.head : document.getElementsByTagName('head')[0]
  ...
  var isSupportCustomEvent = !!window.CustomEvent
  var fireEvent
  ...
  var orientationCB = function (e) {
    ...
    return function () {
      if (win.orientation === 180 || win.orientation === 0) {
        if (meta.current === 'portrait') return
        meta.current = 'portrait'
      }
      if (win.orientation === 90 || win.orientation === -90) {
        if (meta.current === 'landscape') return
        meta.current = 'landscape'
      }
  ...
  var resizeCB = function () {
    ...
    var lstr = 'landscape, ' + ffstr
    var cssstr = '@media (orientation: portrait) { .orientation{font-family:' + pstr + ';} } @media (orientation: landscape) {  .orientation{font-family:' + lstr + ';}}'
